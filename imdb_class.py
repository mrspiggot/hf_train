...
class ModelEvaluator:
    ...
    def evaluate(self, dataset, sample_size=None):
        ...
        precision = self.precision_metric.compute(predictions=predictions, references=sample_labels, average='binary')[
            'precision']
        recall = self.recall_metric.compute(predictions=predictions, references=sample_labels, average='binary')[
            'recall']
...
class SentimentAnalysis:
    def __init__(self, model_name, dataset_name):
        ...
        self.data_preprocessor = DataPreprocessor(dataset_name, self.tokenizer)
        self.model_evaluator = ModelEvaluator(self.model, self.tokenizer)
        self.model_trainer = ModelTrainer(self.model, self.tokenizer, self.data_preprocessor.train_dataset,
                                          self.data_preprocessor.val_dataset)
...
if __name__ == "__main__":
    sentiment_analysis = SentimentAnalysis(model_name='distilbert-base-uncased-finetuned-sst-2-english',
                                           dataset_name='imdb')
